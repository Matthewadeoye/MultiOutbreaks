...
SMOOTHING_INFERENCE<- function(y, e_it, Modeltype, adjmat, step_sizes = list("r"=0.3,"s"=0.3,"u"=0.3), Stan=FALSE,
  ...
  if(Stan){
    ...
    y_total<- ifelse(is.na(y_total),-1,y_total)
    InclusionExclusionSets<- PrecomputeCopulaInclusionExclusion(nstrain)
    maxLength<- InclusionExclusionSets[["n_subsets"]]
    num_subsets<- InclusionExclusionSets[["num_subsets"]]
    subset_sizes<- InclusionExclusionSets[["subset_sizes"]]
    subset_indices<- InclusionExclusionSets[["subset_indices"]]
    OutbreakPrior_ExpectationMatrix<- matrix(c(11/12,1/12,6/12,6/12), nrow = 2, byrow = T)
    DirichletPrior<- 12 * JointTransitionMatrix_arma_cpp(OutbreakPrior_ExpectationMatrix, nstrain)
    npar<- ifelse(Modeltype>0,nstrain,1)
    mod7nstate<- ifelse(Modeltype==7,nstate,1)
    initials <- list(transitionParams = rep(c(0.1, 0.2), npar), u = rep(0, ndept-1), rraw = rep(0, time), sraw = rep(0, 11), kappa_u=20, kappa_r=10, kappa_s=5, B=rep(1.0, npar), a_k=rep(-10, nstrain), lambda_free=rep(0, nstrain-1), frankparam=2.4)
    initials_list <- lapply(1:n_chains, function(x) initials)
    if(GPU){
      mod <- cmdstanr::cmdstan_model(system.file("stan", "MultitypeModel.stan", package = "MultiOutbreaks", mustWork = T), compile = F, cpp_options = list(stan_opencl = TRUE))
      if(verbose){
        mod$compile()
        fit<- mod$sample(data = list(ndept=ndept, time=time, nstate=nstate, rankdef=rankdef, nstrain=nstrain, npar=npar, y=y, e_it=e_it, R=R, Bits=Bits, SMat = RW1PrecMat, Modeltype = Modeltype, gh_x=gh$nodes, gh_w=gh$weights, n_subsets=maxLength, num_subsets=num_subsets, subset_sizes=subset_sizes,subset_indices=subset_indices, y_total=y_total,DirichletPrior=DirichletPrior, mod7nstate=as.integer(mod7nstate)),
                           init = initials_list, chains = n_chains, iter_warmup = round(0.25*HMC_iterations),
                           iter_sampling = round(0.75*HMC_iterations), parallel_chains = n_chains,
                           set.seed(1234), opencl_ids = c(0, 0), adapt_delta = adaptdelta)
      }else{
      ...
      mod <- cmdstanr::cmdstan_model(system.file("stan", "MultitypeModel.stan", package = "MultiOutbreaks", mustWork = T), compile = F)
      if(verbose){
        mod$compile()
        fit<- mod$sample(data = list(ndept=ndept, time=time, nstate=nstate, rankdef=rankdef, nstrain=nstrain, npar=npar, y=y, e_it=e_it, R=R, Bits=Bits, SMat = RW1PrecMat, Modeltype = Modeltype, gh_x=gh$nodes, gh_w=gh$weights, n_subsets=maxLength, num_subsets=num_subsets, subset_sizes=subset_sizes,subset_indices=subset_indices, y_total=y_total,DirichletPrior=DirichletPrior, mod7nstate=as.integer(mod7nstate)),
                           init = initials_list, chains = n_chains, iter_warmup = round(0.25*HMC_iterations),
                           iter_sampling = round(0.75*HMC_iterations), parallel_chains = n_chains,
                           set.seed(1234), adapt_delta = adaptdelta)
      }else{
        invisible(capture.output(suppressMessages({
          mod$compile()
          fit<- mod$sample(data = list(ndept=ndept, time=time, nstate=nstate, rankdef=rankdef, nstrain=nstrain, npar=npar, y=y, e_it=e_it, R=R, Bits=Bits, SMat = RW1PrecMat, Modeltype = Modeltype, gh_x=gh$nodes, gh_w=gh$weights, n_subsets=maxLength, num_subsets=num_subsets, subset_sizes=subset_sizes,subset_indices=subset_indices, y_total=y_total,DirichletPrior=DirichletPrior, mod7nstate=as.integer(mod7nstate)),
  ...
  if(Modeltype %in% c(1,2,5,6)){
    ...
    MC_chain[1,num_Gammas+3+time+12+ndept+nstrain+nstrain+1]<- 0.9999
  ...
  sdGsJoint<- rep(sdGs, nstrain)

  if(Modeltype %in% c(1,2)){
    zigmaJ <- diag(0.0001, num_Gammas+nstrain+nstrain)
  }else if(Modeltype %in% c(3,4)){
    zigmaJ <- diag(0.0001, num_Gammas+nstrain+nstrain+n_factloadings-1)
  }else if(Modeltype %in% c(5,6)){
    ...
    if(Model == 0){
      ...
      if(Modeltype %in% c(1,2)){
        ...
        for(n in 1:nstate){
          ...
          proposalcurrentproposedGs<- log_Dirichlet(MC_chain[i-1, (index:(n*nstate))], JointTPM1[n, ])

          priorcurrentGs<- log_Dirichlet(MC_chain[i-1, (index:(n*nstate))], state_n_Prior)
          priorproposedGs<- log_Dirichlet(JointTPM1[n, ], state_n_Prior)
      ...
      if(Modeltype %in% c(1,2)){
        ...
        if(i==500){
          ...
          XnJ<- cbind(qlogis(MC_chain[half_hist:i, 1:num_Gammas]), MC_chain[half_hist:i, num_Gammas+3+time+12+ndept+(1:nstrain)], MC_chain[half_hist:i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)])
          XnbarJ <- colMeans(XnJ)
          zigmaJ <- cov(XnJ) + epsilonJ * diag(1, num_Gammas+nstrain+nstrain)
          zigmaJ<- optconstantJ * zigmaJ
        } else if (i > 500){
          currentJcomps<- c(qlogis(MC_chain[i, 1:num_Gammas]), MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)], MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)])
          proposedJcomps<- mvnfast::rmvn(1, mu = currentJcomps, sigma = zigmaJ)
          proposedGs<- plogis(proposedJcomps[1:num_Gammas])

          JointTPM1<- Multipurpose_JointTransitionMatrix(proposedGs, nstrain, MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+nstrain+(1:n_copParams)], Modeltype)
          ...
          mh.ratioJ<- exp(likelihoodproposed + priorproposedGs + priorproposedB + priorproposedAks + proposalcurrentJcomps + sum(log(proposedGs * (1 - proposedGs)))
                          - likelihoodcurrent - priorcurrentGs - priorcurrentB - priorcurrentAks - proposalproposedJcomps - sum(log(MC_chain[i,1:num_Gammas] * (1 -  MC_chain[i,1:num_Gammas]))))
          ...
          if(i>500 && i<burn_in && !is.na(mh.ratioJ)){
          currentJcomps<- c(qlogis(MC_chain[i, 1:num_Gammas]), MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)], MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)])
          ...
          lambdaJ<- lambdaJ * exp((2/max(1, i-500)) * (min(mh.ratioJ, 1) - 0.234))
        ...
        if(i==500){
          acc<- 0
          optconstantJ<- 2.38^2/(num_Gammas+nstrain+nstrain+n_factloadings-1)
          lambdaJ<- 1
          epsilonJ<- 1e-6
          half_hist<- floor(0.5*i)
        ...
        if(i==500){
          ...
          epsilonJ<- 1e-6
          half_hist<- floor(0.5*i)
          XnJ<- cbind(qlogis(MC_chain[half_hist:i, 1:num_Gammas]), MC_chain[half_hist:i, num_Gammas+3+time+12+ndept+(1:nstrain)], MC_chain[half_hist:i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)], MC_chain[half_hist:i, ncol(MC_chain)])
          XnbarJ <- colMeans(XnJ)
          zigmaJ<- cov(XnJ) + epsilonJ * diag(1, num_Gammas+nstrain+nstrain+1)
          zigmaJ<- optconstantJ * zigmaJ
        } else if (i > 500){
          currentJcomps<- c(qlogis(MC_chain[i, 1:num_Gammas]), MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)], MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)], MC_chain[i, ncol(MC_chain)])
          proposedJcomps<- mvnfast::rmvn(1, mu = currentJcomps, sigma = zigmaJ)
          proposedGs<- plogis(proposedJcomps[1:num_Gammas])

            JointTPM1<- Multipurpose_JointTransitionMatrix(proposedGs, nstrain, proposedJcomps[length(proposedJcomps)], Modeltype)
            ...
            if(any(!is.finite(JointTPM1))){
              ...
              mh.ratioJ<- exp(likelihoodproposed + priorproposedGs + priorproposedB + priorproposedAks + priorproposedcopPs + proposalcurrentJcomps + sum(log(proposedGs * (1 - proposedGs)))
                              - likelihoodcurrent - priorcurrentGs - priorcurrentB - priorcurrentAks - priorcurrentcopPs - proposalproposedJcomps - sum(log(MC_chain[i,1:num_Gammas] * (1 - MC_chain[i,1:num_Gammas]))))
  ...
  if(Modeltype %in% (1:7) && burn_in>500 && MCMC_iterations>500) print(acc/(MCMC_iterations-500))
...
#Inference Method 1 -- FFBS
NewFFBS_INFERENCE<- function(y, e_it, Modeltype, adjmat, step_sizes = list("r"=0.3,"s"=0.3,"u"=0.3), Stan=FALSE,
                               MCMC_iterations = 15000, HMC_iterations=5000, sdBs=0.003, sdGs=0.005, sdLambdas=0.003, sdCop=0.0002,
                               y_total=NULL, RM_Gs=TRUE, RM_Bs=TRUE, RM_Cop=TRUE, RM_Lambdas=TRUE,
  ...
  if(Stan){
    ...
    if(Modeltype %in% c(1,2,5,6)){
      ...
      eta<- eta[-1]
      eta_matrix<- matrix(NA, nrow = MCMC_iterations, ncol = n_factloadings-1)
      eta_matrix[1,]<- eta
      JointTPM<- Multipurpose_JointTransitionMatrix2(MC_chain[1,1:num_Gammas], nstrain, MC_chain[1,num_Gammas+3+time+12+ndept+nstrain+nstrain+(1:n_factloadings)], Modeltype, gh)
      if(any(!is.finite(JointTPM))) JointTPM<- initGs
    }else if(Modeltype == 7){
    ...
    for (i in 2:MCMC_iterations) {
      ...
      Allquantities<- FFBSgradmultstrainLoglikelihood_cpp(y=y, e_it=e_it, nstrain=nstrain,  r=MC_chain[i, num_Gammas+3+(1:time)], s=MC_chain[i, num_Gammas+3+time+(1:12)], u=MC_chain[i, num_Gammas+3+time+12+(1:ndept)], jointTPM=JointTPM, B=MC_chain[i-1, num_Gammas+3+time+12+ndept+(1:nstrain)], Bits=Bits, a_k=proposedAks, Model=Model,Q_r=Q_r,Q_s = Q_s,Q_u=Q_u,gradients=1,Qstz_r=Qstz_r, Qstz_s=Qstz_s, Qstz_u=Qstz_u, y_total = y_total)
      grad_proposed <- list(grad_r=as.numeric(Allquantities$grad_r), grad_s=as.numeric(Allquantities$grad_s), grad_u=as.numeric(Allquantities$grad_u), cov_r=Allquantities$cov_r, cov_s=Allquantities$cov_s, cov_u=Allquantities$cov_u)
      ...
      if(Model == 0){
        ...
        Allquantities<- FFBSgradmultstrainLoglikelihood_cpp(y=y, e_it=e_it, nstrain=nstrain,  r=MC_chain[i, num_Gammas+3+(1:time)], s=MC_chain[i, num_Gammas+3+time+(1:12)], u=MC_chain[i, num_Gammas+3+time+12+(1:ndept)], jointTPM=JointTPM, B=proposedB, Bits=Bits, a_k=MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)], Model=Model,Q_r=Q_r,Q_s = Q_s,Q_u=Q_u,gradients=0,Qstz_r=Qstz_r, Qstz_s=Qstz_s, Qstz_u=Qstz_u, y_total = y_total)
        ...
        if(Modeltype %in% c(1,2)){
          proposedGs<- rnorm(num_Gammas,mean=qlogis(MC_chain[i-1,1:num_Gammas]), sd=rep(sdGs, num_Gammas))
          proposedGs<- plogis(proposedGs)

          proposedLambdas<- rep(0, nstrain)
          JointTPM1<- Multipurpose_JointTransitionMatrix(proposedGs, nstrain, proposedLambdas, Modeltype)

          Allquantities<- FFBSgradmultstrainLoglikelihood_cpp(y=y, e_it=e_it, nstrain=nstrain,  r=MC_chain[i, num_Gammas+3+(1:time)], s=MC_chain[i, num_Gammas+3+time+(1:12)], u=MC_chain[i, num_Gammas+3+time+12+(1:ndept)], jointTPM=JointTPM1, B=MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)], Bits=Bits, a_k=MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)], Model=Model,Q_r=Q_r,Q_s = Q_s,Q_u=Q_u,gradients=1,Qstz_r=Qstz_r, Qstz_s=Qstz_s, Qstz_u=Qstz_u, y_total = y_total)
          grad_proposed <- list(grad_r=as.numeric(Allquantities$grad_r), grad_s=as.numeric(Allquantities$grad_s), grad_u=as.numeric(Allquantities$grad_u), cov_r=Allquantities$cov_r, cov_s=Allquantities$cov_s, cov_u=Allquantities$cov_u)
          ...
          mh.ratio<- exp(likelihoodproposed + priorproposedGs + sum(log(proposedGs * (1 - proposedGs)))
                         - likelihoodcurrent - priorcurrentGs - sum(log(MC_chain[i-1, 1:num_Gammas] * (1 - MC_chain[i-1, 1:num_Gammas]))))
          ...
          if(RM_Gs && i<burn_in && !is.na(mh.ratio)) {sdGs= sdGs * exp((RMdelta/i) * (min(mh.ratio, 1) - 0.234))}
            ...
            if(any(!is.finite(JointTPM1))){
              ...
              mh.ratioGC<- exp(likelihoodproposed + priorproposedGs + sum(log(proposedGs * (1 - proposedGs)))
                               - likelihoodcurrent - priorcurrentGs - sum(log(MC_chain[i-1, 1:num_Gammas] * (1 - MC_chain[i-1, 1:num_Gammas]))))
          ...
          Allquantities<- FFBSgradmultstrainLoglikelihood_cpp(y=y, e_it=e_it, nstrain=nstrain,  r=MC_chain[i, num_Gammas+3+(1:time)], s=MC_chain[i, num_Gammas+3+time+(1:12)], u=MC_chain[i, num_Gammas+3+time+12+(1:ndept)], jointTPM=JointTPM, B=MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)], Bits=Bits, a_k=MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)], Model=Model,Q_r=Q_r,Q_s = Q_s,Q_u=Q_u,gradients=1,Qstz_r=Qstz_r, Qstz_s=Qstz_s, Qstz_u=Qstz_u, y_total=y_total)
          grad_current <- list(grad_r=as.numeric(Allquantities$grad_r), grad_s=as.numeric(Allquantities$grad_s), grad_u=as.numeric(Allquantities$grad_u), cov_r=Allquantities$cov_r, cov_s=Allquantities$cov_s, cov_u=Allquantities$cov_u)
          likelihoodcurrent<- Allquantities$loglike
        }else if(Modeltype %in% c(5,6)){
          proposedGs<- rnorm(num_Gammas,mean=qlogis(MC_chain[i-1,1:num_Gammas]), sd=rep(sdGs, num_Gammas))
        ...
        if(Modeltype %in% c(1,2)){
          ...
          if(i==500){
            ...
            if(any(!is.finite(JointTPM1))){
              ...
              Allquantities<- FFBSgradmultstrainLoglikelihood_cpp(y=y, e_it=e_it, nstrain=nstrain,  r=MC_chain[i, num_Gammas+3+(1:time)], s=MC_chain[i, num_Gammas+3+time+(1:12)], u=MC_chain[i, num_Gammas+3+time+12+(1:ndept)], jointTPM=JointTPM1, B=proposedJcomps[num_Gammas+(1:nstrain)], Bits=Bits, a_k=proposedJcomps[num_Gammas+nstrain+(1:nstrain)], Model=Model,Q_r=Q_r,Q_s = Q_s,Q_u=Q_u, gradients=1,Qstz_r=Qstz_r, Qstz_s=Qstz_s, Qstz_u=Qstz_u, y_total = y_total)
              grad_proposed <- list(grad_r=as.numeric(Allquantities$grad_r), grad_s=as.numeric(Allquantities$grad_s), grad_u=as.numeric(Allquantities$grad_u), cov_r=Allquantities$cov_r, cov_s=Allquantities$cov_s, cov_u=Allquantities$cov_u)
              likelihoodproposed<- Allquantities$loglike
          ...
          if(i==500){
            ...
            epsilonJ<- 1e-6
            half_hist<- floor(0.5*i)
            XnJ<- cbind(qlogis(MC_chain[half_hist:i, 1:num_Gammas]), MC_chain[half_hist:i, num_Gammas+3+time+12+ndept+(1:nstrain)], MC_chain[half_hist:i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)], eta_matrix[half_hist:i,])
            XnbarJ <- colMeans(XnJ)
            zigmaJ <- cov(XnJ) + epsilonJ * diag(1, num_Gammas+nstrain+nstrain+n_factloadings-1)
            zigmaJ<- optconstantJ * zigmaJ
          } else if (i > 500){
            currentJcomps<- c(qlogis(MC_chain[i, 1:num_Gammas]), MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)], MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)], eta_matrix[i,])
            proposedJcomps<- mvnfast::rmvn(1, mu = currentJcomps, sigma = zigmaJ)
            LAMBDAS_prop <- 0.99*tanh(proposedJcomps[num_Gammas+nstrain+nstrain+(1:(n_factloadings-1))])
            LAMBDAS_prop<- c(1, LAMBDAS_prop)
            proposedGs<- plogis(proposedJcomps[1:num_Gammas])
            ...
            if(any(!is.finite(JointTPM1))){
              ...
              mh.ratioJ<- exp(likelihoodproposed + priorproposedGs + priorproposedB + priorproposedAks +  priorproposedLAMBDA + proposalcurrentJcomps + sum(log(proposedGs * (1 - proposedGs)))
                              - likelihoodcurrent - priorcurrentGs - priorcurrentB - priorcurrentAks - priorcurrentLAMBDA - proposalproposedJcomps - sum(log(MC_chain[i,1:num_Gammas] * (1 - MC_chain[i,1:num_Gammas]))))
              ...
              if(!is.na(mh.ratioJ) && runif(1) < mh.ratioJ){
                ...
                likelihoodcurrent<- likelihoodproposed
                acc<- acc + 1
                eta_matrix[i,]<- proposedJcomps[num_Gammas+nstrain+nstrain+(1:(n_factloadings-1))]
              }else{
                MC_chain[i,1:num_Gammas]<- MC_chain[i,1:num_Gammas]
                MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)]<- MC_chain[i, num_Gammas+3+time+12+ndept+(1:nstrain)]
                MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)]<- MC_chain[i, num_Gammas+3+time+12+ndept+nstrain+(1:nstrain)]
          ...
          if(i==500){
            ...
            Allquantities<- FFBSgradmultstrainLoglikelihood_cpp(y=y, e_it=e_it, nstrain=nstrain,  r=MC_chain[i, num_Gammas+3+(1:time)], s=MC_chain[i, num_Gammas+3+time+(1:12)], u=MC_chain[i, num_Gammas+3+time+12+(1:ndept)], jointTPM=JointTPM, B=proposedJcomps[1:nstrain], Bits=Bits, a_k=proposedJcomps[nstrain+(1:nstrain)], Model=Model,Q_r=Q_r,Q_s = Q_s,Q_u=Q_u, gradients=1,Qstz_r=Qstz_r, Qstz_s=Qstz_s, Qstz_u=Qstz_u, y_total = y_total)
            grad_proposed <- list(grad_r=as.numeric(Allquantities$grad_r), grad_s=as.numeric(Allquantities$grad_s), grad_u=as.numeric(Allquantities$grad_u), cov_r=Allquantities$cov_r, cov_s=Allquantities$cov_s, cov_u=Allquantities$cov_u)
            likelihoodproposed<- Allquantities$loglike
